    ...
    if mission.Taken and mission.Completion == COMMON.MISSION_INCOMPLETE and zoneId == mission.Zone and segmentID == mission.Segment and mission.Client ~= "" then
      if mission.Type == COMMON.MISSION_TYPE_ESCORT or mission.Type == COMMON.MISSION_TYPE_EXPLORATION then -- escort
        ...
        if player_count + guest_count >= 4 then
          ...
          while state > -1 do
            UI:ResetSpeaker()
            UI:WaitShowDialogue("Demandez à l'un des membres de votre équipe de revenir dans la guilde pour faire de la place à votre client, " .. _DATA:GetMonster(mission.Client):GetColoredName() .. ".")
        ...
        tbl.Escort = name
        UI:ResetSpeaker()
        UI:WaitShowDialogue("Ajout de[color=#00FF00]" .. new_mob.Name .. "[color]à la fête en tant qu'invité.")
      end
    end
  end
